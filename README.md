    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as user_socket:
        ...
        with conn:
            ...
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as receiver_socket:
                ...
                receiver_socket.sendall(transformed_message.encode())  # Ensure full message is sent
...
class ChatClient:
    def __init__(self, root, client):
        ...
        self.chat_area = scrolledtext.ScrolledText(self.root, width=60, height=20, wrap="word")
    ...
    def display_message(self, message):
        ...
        if message.startswith("\n") or message.startswith("Welcome") or message.startswith("Server"):
...
def send_packets(window_size, total_packets, loss_probability):
    ...
    while base < total_packets:
        print(f"Sending packets in window: {list(range(base, min(base + window_size, total_packets)))}")
